...
sub _CoreAccessible {
    {
        ...
        eval "require RTx::AssetTracker::Scrip_Overlay";
        if ($@ && $@ !~ qr{^Can't locate RTx/AssetTracker/Scrip_Overlay.pm}) {
            die $@;
        };

        eval "require RTx::AssetTracker::Scrip_Vendor";
        if ($@ && $@ !~ qr{^Can't locate RTx/AssetTracker/Scrip_Vendor.pm}) {
            die $@;
        };

        eval "require RTx::AssetTracker::Scrip_Local";
        if ($@ && $@ !~ qr{^Can't locate RTx/AssetTracker/Scrip_Local.pm}) {
            die $@;
        };
...
RTx::AssetTracker::Scrip_Overlay, RTx::AssetTracker::Scrip_Vendor, RTx::AssetTracker::Scrip_Local
