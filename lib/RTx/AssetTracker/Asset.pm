...
Takes a paramhash of Type and one of Base or Target. Removes that link from this asset.
...
Calls _DeleteLink silently, create transactions ourselves so we can implement change comments.
...
sub DeleteLink {
    ...
    my ($ret, $TransString) = $self->_DeleteLink( %args, Silent => 1 );
    return (0, $TransString) unless $ret;
...
Calls _AddLink silently, create transactions ourselves so we can implement change comments.
...
sub AddLink {
    ...
    my ($linkid, $TransString) = $self->_AddLink( %args, Silent => 1 );
    return (0, $TransString) unless $linkid;
