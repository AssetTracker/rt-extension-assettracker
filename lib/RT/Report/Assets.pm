...
use base qw/RTx::AssetTracker::Assets/;
...
sub Groupings {
    ...
    my %args = (@_);
    my @fields =
      map { $self->CurrentUser->loc($_), $_ } qw( Status Type );    # loc_qw

    foreach my $type ( qw(Creator LastUpdatedBy Owner Admin Watcher) ) { # loc_qw
        for my $field (
    ...
    for my $field (qw(Created LastUpdated)) { # loc_qw
...
sub _FieldToFunction {
    ...
    if ($field =~ /^(.*)(Hourly|Daily|Monthly|Annually)$/) {
        ...
        unless ( $cf->id ) {
            ...
            my ($asset_cf_alias, $cf_alias) = $self->_CustomFieldJoin($cf->id, $cf->id, $cf_name);
            @args{qw(ALIAS FIELD)} = ($asset_cf_alias, 'Content');
        }
    } elsif ( $field =~ /^(?:(Creator|LastUpdatedBy))(?:\.(.*))?$/ ) {
        my $type = $1 || '';
        my $column = $2 || 'Name';
    ...
    } elsif ( $field =~ /^(?:Watcher|(Owner|Admin))(?:\.(.*))?$/ ) {
        my $type = $1 || '';
        my $column = $2 || 'Name';
...
# Gotta skip over RTx::AssetTracker::Assets->Next, since it does all sorts of crazy magic we 
# don't want.
sub Next {
    my $self = shift;
...
sub NewItem {
    my $self = shift;
    return RT::Report::Assets::Entry->new(RT->SystemUser); # $self->CurrentUser);
}
...
sub AddEmptyRows {
    ...
    if ( @{ $self->{'_group_by_field'} || [] } == 1 && $self->{'_group_by_field'}[0] eq 'Status' ) {
        ...
        foreach my $status ( grep !$has{$_}, RTx::AssetTracker::Type->new($self->CurrentUser)->StatusArray ) {
