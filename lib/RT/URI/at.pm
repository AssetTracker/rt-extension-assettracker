...
When handed an at: URI, figures out things like whether its a local record and what its ID is
...
sub HREF {
    ...
    my $obj = $self->Object;
    if ( $obj && $self->ObjectType eq 'asset' ) {
        return RT->Config->Get('WebURL') ."AssetTracker/Asset/Display.html?id=". $obj->id;
    }
...
Returns either a localized string C<ObjectType #13: Name> or the full URI if the object
is not local.
