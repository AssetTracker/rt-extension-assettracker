...
package RT::Shredder::Plugin::AssetObjects;
...
RT::Shredder::Plugin::AssetObjects - search plugin for wiping any selected object.
...
sub SupportArgs
{
    return $_[0]->SUPER::SupportArgs, qw(Asset IP Port);
}
...
sub TestArgs
{
    ...
    my @strings;
    foreach my $name( qw(Asset IP Port) ) {
        next unless $args{$name};

        my $list = $args{$name};
        $list = [$list] unless UNIVERSAL::isa( $list, 'ARRAY' );
        push @strings, map "RTx::AssetTracker::$name\-$_", @$list;
    }
